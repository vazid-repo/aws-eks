resource "aws_security_group" "staging_eks_alb_sg" {
  description = "Allow worker nodes pods to communicate with outsiders"
  vpc_id      = "${var.vpc_id}"
  name        = "staging-eks-alb-loadbalancer-sg"
...
resource "aws_security_group_rule" "allow_http" {
  ...
  to_port                  = 80
  protocol                 = "tcp"
  security_group_id        = "${aws_security_group.staging_eks_alb_sg.id}"
